<?php
/**
 * RozetkaPay Payparts Gateway Class - АДАПТИРОВАНА ПОД НОВОЕ API
 * Версия 2.0.3-native-api
 * 
 * Изменения:
 * - Прямая работа с новым API форматом available_periods
 * - Убрана конвертация форматов данных
 * - Сохранена полная стилистика и функциональность
 * - Адаптация под изменения RozetkaPay API от января 2026
 */
    ...
    private function get_installment_options() {
        ...
        if ( is_array( $banks ) ) {
            foreach ( $banks as $bank ) {
                ...
                // Обробляємо періоди - адаптація до нового API формату
                $periods = array();
                
                if ( ! empty( $bank['available_periods'] ) && is_array( $bank['available_periods'] ) ) {
                    foreach ( $bank['available_periods'] as $period ) {
